...
showWinScreen() {
  ...
  if (isNewHigh) {
    ...
    const newHighText = this.add.text(W / 2, yPos, "🌟 NEW HIGH SCORE! 🌟", {
      fontSize: "26px",
      color: "#ffdd00",
      fontStyle: "bold"
    }).setOrigin(0.5);
  ...
  if (!isLastLevel) {
    // Next level button
    const nextBtn = this.add.text(W / 2, yPos, "▶  Next Level", {
      fontSize: "26px",
      color: "#ffffff",
      fontStyle: "bold",
  ...
  // Level select button
  const selectBtn = this.add.text(W / 2, yPos, "☰  Level Select", {
    fontSize: "20px",
    color: "#ffffff",
  ...
  // Menu button
  const menuBtn = this.add.text(W / 2, yPos, "⌂  Main Menu", {
    fontSize: "20px",
    color: "#aaaaaa",
    padding: { x: 16, y: 8 }
  }).setOrigin(0.5).setInteractive({ useHandCursor: true });
